...
// ==========================================
// 1. DATA STRUCTURES & GRAPH FOUNDATION
// ==========================================
...
class MoveRecord {
    int startX, startY, endX, endY, capX, capY;
    ...
    public MoveRecord(int sx, int sy, int ex, int ey, Piece m, Piece c, int cx, int cy, boolean promo, boolean castle, boolean hadMoved) {
        ...
        moved = m; captured = c; capX = cx; capY = cy;
        wasPawnPromotion = promo; wasCastling = castle; movedHadMoved = hadMoved;
    }
}
...
class BoardGraph {
    Square[][] grid;
    Stack<MoveRecord> moveHistory = new Stack<>();
    Stack<MoveRecord> redoStack = new Stack<>();
    ...
    public void setupStandardBoard() {
        moveHistory.clear(); redoStack.clear();
        for(int x=0; x<8; x++) {
            for(int y=0; y<8; y++) grid[x][y].setPiece(null); 
        }
        for (int x = 0; x < 8; x++) {
...
// ==========================================
// 2. PIECE ECOSYSTEM (WITH EN PASSANT)
// ==========================================
...
class MoveValidator {
    ...
    public static boolean isMoveStrictlyLegal(BoardGraph board, Square start, Square target, Color movingColor) {
        ...
        if (isCastling) {
            ...
            transitSquare = board.getSquare(start.getX() + dir, start.getY());
        }
        ...
        if (movingPiece instanceof Pawn && target.isEmpty() && start.getX() != target.getX()) {
            ...
            epCapturedPiece = epCaptureSquare.getPiece();
        ...
        if (epCaptureSquare != null) epCaptureSquare.setPiece(epCapturedPiece); 
        ...
        for (int y = 0; y < 8; y++) {
            for (int x = 0; x < 8; x++) {
                ...
                if (!sq.isEmpty() && sq.getPiece().getColor() == currentTurn) {
                    if (sq.getPiece() instanceof King) kingVertex = sq;
...
public class DMGTChess extends JFrame {
    ...
    private void performUndo() {
        ...
        startSq.getPiece().hasMoved = last.movedHadMoved; 
        endSq.setPiece(null);
